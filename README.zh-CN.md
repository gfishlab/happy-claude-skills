...
### agent-init
为 Codex 或 Claude Code 初始化轻量项目协作骨架。生成入口文件（`AGENTS.md`/`CLAUDE.md`）及运行目录，包含 rules、hooks、memory、subagents 模板。

**适用场景：**
- 新项目快速搭建 agent 协作骨架
- 已有项目补齐缺失的规则、钩子或记忆模板
- 团队项目结构标准化（支持 Codex 和 Claude Code）

**核心功能：**
- 支持 Codex（`AGENTS.md` + `.codex/`）和 Claude Code（`CLAUDE.md` + `.claude/`）
- 幂等执行：复用已有文件，仅补齐缺失部分
- 生成结构化运行目录：rules、hooks、memory、subagents
